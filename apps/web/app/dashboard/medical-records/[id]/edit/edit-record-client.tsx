...
import { Save, FileText, User, Microscope, Pill, ListChecks, Info, ChevronLeft, Activity } from "lucide-react";
import { useState } from "react";
import { cn } from "@/lib/utils";
import { useRouter } from "next/navigation";
import { updateMedicalRecord } from "@/app/actions/medical-records";
...
export default function EditRecordClient({ record, lookups }: { record: any, lookups: any[] }) {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState("subjective");
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Normalize objective structure if it was saved as an object or string
  const initialObjective = typeof record.objective === 'string' ? record.objective : (record.objective as any)?.findings || "";
  const initialVitals = typeof record.objective === 'object' ? (record.objective as any)?.vitals || {} : {};
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      router.push(`/dashboard/patients/${record.patientId}`);
    } catch (error: any) {
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-black tracking-tight text-foreground">Edit Clinical Note</h2>
            <div className="flex items-center gap-2 text-sm text-muted-foreground font-medium">
               <span>Updating record for:</span>
               <span className="font-black text-primary uppercase underline decoration-2 underline-offset-4">{record.patient.fullName}</span>
            </div>
          </div>
        </div>
        <button 
          ...
          {sections.map((s) => (
            ...
            >
              ...
              <div className="space-y-1">
                 <div className={cn("text-[10px] font-black uppercase tracking-[0.1em]", activeTab === s.id ? "text-primary font-black" : "text-muted-foreground")}>{s.label}</div>
               ...
               {['subjective', 'assessment', 'plan'].includes(activeTab) ? (
                 ...
                 <div className="flex flex-col h-full">
                    ...
                    <textarea 
                       ...
                       className="w-full flex-1 text-base font-medium p-8 focus:outline-none resize-none bg-transparent min-h-[300px] text-foreground leading-relaxed"
                    />
                 </div>
               ) : activeTab === 'rx' ? (
                 <div className="p-8">
                    <PrescriptionForm lookups={lookups} onUpdate={(rx: any) => form.setValue('prescriptions', rx)} initialData={record.prescriptions} />
                 </div>
               ) : activeTab === 'labs' ? (
                 <div className="p-8">
                    <LabTestRequest patientName={record.patient.fullName} lookups={lookups} onUpdate={(labs: any) => form.setValue('labRequests', labs)} initialData={record.labRequests?.filter((l: any) => l.type !== "RADIOLOGY")} />
                 </div>
               ) : activeTab === 'radiology' ? (
                 <div className="p-8">
                    <RadiologyRequest patientName={record.patient.fullName} lookups={lookups} onUpdate={(rads: any) => form.setValue('radiologyRequests', rads)} initialData={record.labRequests?.filter((l: any) => l.type === "RADIOLOGY")} />
                 </div>
               ) : (
                 <div className="p-8">
